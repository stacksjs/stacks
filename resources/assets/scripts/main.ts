...
// Window dragging state (declared early to avoid TDZ issues)
...
// Desktop icon data - set data attributes via JS since STX variable interpolation has issues
const desktopIconData: Record<string, { section?: string; url?: string; left: string; top: string }> = {
  'welcome': { section: 'welcome', left: '20px', top: '20px' },
  'about': { section: 'about', left: '140px', top: '20px' },
  'ecosystem': { section: 'ecosystem', left: '260px', top: '20px' },
  'backend': { section: 'backend', left: '380px', top: '20px' },
  'frontend': { section: 'frontend', left: '500px', top: '20px' },
  'cloud': { section: 'cloud', left: '620px', top: '20px' },
  'dashboard': { section: 'dashboard', left: '20px', top: '140px' },
  'license': { section: 'license', left: '140px', top: '140px' },
  'login': { left: '260px', top: '140px' },
  'benchmarks': { section: 'benchmarks', left: '380px', top: '140px' },
  'libraries': { section: 'libraries', left: '500px', top: '140px' },
  'plugins': { section: 'plugins', left: '620px', top: '140px' },
  'templates': { section: 'templates', left: '20px', top: '260px' },
  'docs': { url: 'https://stacksjs.com/docs', left: '140px', top: '260px' },
...
// Apply icon data attributes on page load
document.querySelectorAll('.desktop-icon').forEach((icon) => {
  const id = (icon as HTMLElement).dataset.iconId
  if (id && desktopIconData[id]) {
    const data = desktopIconData[id]
    if (data.section) (icon as HTMLElement).dataset.iconSection = data.section
    if (data.url) (icon as HTMLElement).dataset.iconUrl = data.url
    ;(icon as HTMLElement).style.left = data.left
    ;(icon as HTMLElement).style.top = data.top
  }
})
...
function updateTaskbar() {
  const taskbarTasks = document.getElementById('taskbar-windows')
