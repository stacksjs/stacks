...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} query log pruning deletes the rows past retention`, async () => {
    const url = dialect === 'sqlite' ? undefined : new URL(connection!)
    if (url && !['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname))
      throw new Error('Query log prune tests require a local disposable database server')
    const directory = await mkdtemp(join(tmpdir(), 'stacks-query-log-prune-'))
    const name = `stacks_query_log_prune_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/query-log-prune.ts`], {
        env: {
          ...process.env, APP_ENV: 'test', DB_CONNECTION: dialect, DB_QUERY_LOGGING_ENABLED: 'false',
          DB_DATABASE_PATH: dialect === 'sqlite' ? join(directory, 'query-log-prune.sqlite') : ':memory:',
          STACKS_QUERY_LOG_PRUNE_CONFIG: config,
      ...
      try {
        ...
        expect(timedOut, 'query log prune must finish without a watchdog kill').toBe(false)
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('query log prune OK')
      }
