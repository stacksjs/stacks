...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} team writes that guard on state do not read an affected-row count as the verdict`, async () => {
    const url = dialect === 'sqlite' ? undefined : new URL(connection!)
    if (url && !['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname))
      throw new Error('Team state write tests require a local disposable database server')
    const directory = await mkdtemp(join(tmpdir(), 'stacks-team-state-writes-'))
    const name = `stacks_team_state_writes_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      // an empty array: `preload = []` is rejected by Bun before 1.4, which is
      // still what a stale local install runs.
      await writeFile(config, '# no preload\n')
      if (admin) { await admin.unsafe(`CREATE DATABASE ${quoted}`); created = true }
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/team-state-writes.ts`], {
        env: {
          ...process.env, APP_ENV: 'test', DB_CONNECTION: dialect, DB_QUERY_LOGGING_ENABLED: 'false',
          DB_DATABASE_PATH: dialect === 'sqlite' ? join(directory, 'team-state-writes.sqlite') : ':memory:',
          STACKS_TEAM_STATE_WRITES_CONFIG: config,
      ...
      try {
        ...
        expect(timedOut, 'team state writes must finish without a watchdog kill').toBe(false)
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('team state writes OK')
      }
