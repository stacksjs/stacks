...
describe('dashboard native STX bindings', () => {
  ...
  test('settings editor uses reactive keyed drafts and native models', () => {
    ...
    expect(page).toContain('<template :if="field.type === \'boolean\' && field.editable">')
    expect(page).toContain('<template :else-if="field.editable">')
    expect(page).not.toContain("@if(field.type === 'boolean'")
