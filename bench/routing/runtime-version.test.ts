...
describe('benchmark runtime requirements', () => {
  test('detects an older runtime and preserves an exact project requirement', async () => {
    writeFileSync(join(root, 'package.json'), JSON.stringify({ engines: { bun: '1.4.2' } }))
    expect(await readRuntimeRequirement(root, '1.3.14')).toEqual({ range: '1.4.2', matches: false })
    expect(await readRuntimeRequirement(root, '1.4.1')).toEqual({ range: '1.4.2', matches: false })
    expect(await readRuntimeRequirement(root, '1.4.2')).toEqual({ range: '1.4.2', matches: true })
  })
  ...
  test('does not invent a requirement for a project without one', async () => {
    ...
    expect(await readRuntimeRequirement(root, '1.4.2')).toBeUndefined()
    expect(runtimeMismatchWarning(undefined, '1.4.2')).toBeUndefined()
    expect(runtimeMismatchWarning({ range: '^1.4.0', matches: true }, '1.4.2')).toBeUndefined()
  })
