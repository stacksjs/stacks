...
describe('benchmark server isolation', () => {
  ...
  it('keeps tuned settings opt-in and prevents shell settings from changing stock profiles', () => {
    const keys = ['BENCH_MODE', 'BENCH_ROUTER_ENTRY', 'BENCH_SQLITE_PROFILE', 'STACKS_SECURITY_HEADERS_DISABLE', 'BENCH_SCENARIO', 'DB_CONNECTION'] as const
    const previous = keys.map(key => process.env[key])
    try {
      process.env.BENCH_MODE = 'minimal'
      process.env.BENCH_ROUTER_ENTRY = 'root'
      ...
      expect(stock.BENCH_ROUTER_ENTRY).toBe('runtime')
      ...
      expect(serverEnvironment(targetById('stacks-root')!, true).BENCH_ROUTER_ENTRY).toBe('root')
      expect(DEFAULT_TARGETS.map(target => target.id)).not.toContain('stacks-root')
    }
    finally {
