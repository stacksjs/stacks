...
import { getNameFromFilePath, matchGlobs, normalizeComponentInfo, parseId, pascalCase, resolveAlias } from '../core/utils'
...
const debug = {
  components: Debug('stacks:context:components'),
  search: Debug('stacks:context:search'),
  hmr: Debug('stacks:context:hmr'),
  declaration: Debug('stacks:declaration'),
  env: Debug('stacks:env'),
}
...
export class Context {
    ...
    Array
      ...
      .forEach((path) => {
        ...
        if (this._componentNameMap[name] && !this.options.allowOverrides) {
          console.warn(`[stacks] component "${name}"(${path}) has naming conflicts with other components, ignored.`)
          return
        }
  ...
  async findComponent(name: string, type: 'component', excludePaths: string[] = []): Promise<ComponentInfo | undefined> {
    // resolve from fs
