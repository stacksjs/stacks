...
function formatPhoneNumber(event: FocusEvent | any) {
  ...
  if (phoneNumber.length === 10)
    user.value.phone = `(${phoneNumber.substring(0, 3)}) ${phoneNumber.substring(3, 6)}-${phoneNumber.substring(6, 10)}`

  else if (phoneNumber.length === 11 && phoneNumber[0] === '1')
    user.value.phone = `1 (${phoneNumber.substring(1, 4)}) ${phoneNumber.substring(4, 7)}-${phoneNumber.substring(7, 11)}`
...
async function createUser() {
  try {
    ...
    await toast.success({ text: 'Successfully created user!' })
