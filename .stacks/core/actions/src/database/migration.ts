...
for (let modelIndex = 0; modelIndex < modelEntity.length; modelIndex++) {
  ...
  if (modelElement.default)
    characteristic += `col.defaultTo('${modelElement.default}')`
  else
    characteristic += 'col'
  ...
  for (let fieldIndex = 0; fieldIndex < modelElement.fieldArray.length; fieldIndex++) {
    ...
    if (fieldAssociation[fieldArrayElement.entity] === 'varchar' && !isMaxLengthPresent)
      entity += '(255)'
  }

  writer.write(`    .addColumn('${modelElement.field}', '${entity}', (col) => ${characteristic})\n`)
}
