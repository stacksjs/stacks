<template>
  <div
    class="px-4 sm:px-6 lg:px-8 py-8"
  >
    <div
      class="flex items-start"
    >
      <div
        class="w-full"
      >
        <section>
          <div>
            <div
              class="overflow-hidden bg-gray-200 divide-y divide-gray-200 rounded-lg dark:bg-gray-600 dark:divide-gray-600 sm:grid sm:grid-cols-2 sm:gap-px sm:divide-y-0"
            >
              <div
                class="relative p-6 bg-white dark:bg-gray-700 group"
              >
                <div
                  class="flex items-center space-x-2"
                >
                  <div
                    class="w-3 h-3 mt-0.5 rounded-full bg-green-500"
                  /><h3
                    class="text-lg"
                  >
                    <a
                      href="#"
                      class="font-semibold focus:outline-none hover:text-home-teal-button"
                    > Uptime </a>
                  </h3>
                </div>
                <div
                  class="mt-8"
                >
                  <p
                    class="mt-2 text-sm text-gray-500 dark:text-gray-400"
                    data-v-85662274=""
                  >
                    Your site is up. We last checked less than a minute ago.
                  </p><div data-v-85662274="">
                    <!----><dl
                      class="grid grid-cols-1 mt-5 overflow-hidden bg-white border divide-y divide-gray-200 rounded-lg dark:bg-gray-600 dark:divide-gray-500 dark:border-gray-500 md:grid-cols-3 md:divide-y-0 md:divide-x"
                      data-v-85662274=""
                    >
                      <div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          Past 7 Days
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            100%
                          </div>
                        </dd>
                      </div><div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          Past 14 Days
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            100%
                          </div>
                        </dd>
                      </div><div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          Past 30 Days
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            100%
                          </div>
                        </dd>
                      </div>
                    </dl>
                  </div>
                </div>
              </div><div
                class="relative p-6 bg-white dark:bg-gray-700 group"
                data-v-85662274=""
              >
                <div
                  class="flex items-center space-x-2"
                  data-v-85662274=""
                >
                  <div
                    class="w-3 h-3 mt-0.5 rounded-full bg-green-500"
                    data-v-85662274=""
                  /><h3
                    class="text-lg"
                    data-v-85662274=""
                  >
                    <a
                      href="#"
                      class="font-semibold focus:outline-none hover:text-home-teal-button"
                      data-v-85662274=""
                    > Performance </a>
                  </h3>
                </div><div
                  class="mt-8"
                  data-v-85662274=""
                >
                  <p
                    class="mt-2 text-sm text-gray-500 dark:text-gray-400"
                    data-v-85662274=""
                  >
                    We're monitoring the performance of your site. Your site is fast. We last checked 3 minutes ago.
                  </p>
                </div>
              </div><div
                class="relative p-6 bg-white dark:bg-gray-700 group"
                data-v-85662274=""
              >
                <div
                  class="flex items-center space-x-2"
                  data-v-85662274=""
                >
                  <div
                    class="w-3 h-3 mt-0.5 rounded-full bg-green-500"
                    data-v-85662274=""
                  /><h3
                    class="text-lg"
                    data-v-85662274=""
                  >
                    <a
                      href="#"
                      class="font-semibold focus:outline-none hover:text-home-teal-button"
                      data-v-85662274=""
                    > Certificate health </a>
                  </h3>
                </div><div
                  class="mt-8"
                  data-v-85662274=""
                >
                  <p
                    class="mt-2 text-sm text-gray-500 dark:text-gray-400"
                    data-v-85662274=""
                  >
                    Your certificate is healthy. We last checked 3 minutes ago.
                  </p><!----><div data-v-85662274="">
                    <dl
                      class="mt-5 overflow-hidden bg-white border divide-y divide-gray-200 rounded-lg dark:divide-gray-500 dark:bg-gray-600 dark:border-gray-500"
                      data-v-85662274=""
                    >
                      <div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          Expires In
                        </dt><dd
                          class="mt-1"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            Will not expire in the next 14 days
                          </div>
                        </dd>
                      </div>
                    </dl>
                  </div>
                </div><span
                  class="absolute text-gray-300 pointer-events-none top-6 right-6 group-hover:text-gray-400"
                  aria-hidden="true"
                  data-v-85662274=""
                ><svg
                  class="w-6 h-6"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  data-v-85662274=""
                ><path
                  d="M20 4h1a1 1 0 00-1-1v1zm-1 12a1 1 0 102 0h-2zM8 3a1 1 0 000 2V3zM3.293 19.293a1 1 0 101.414 1.414l-1.414-1.414zM19 4v12h2V4h-2zm1-1H8v2h12V3zm-.707.293l-16 16 1.414 1.414 16-16-1.414-1.414z"
                  data-v-85662274=""
                /></svg></span>
              </div><div
                class="relative p-6 bg-white dark:bg-gray-700 group"
                data-v-85662274=""
              >
                <div
                  class="flex items-center space-x-2"
                  data-v-85662274=""
                >
                  <div
                    class="w-3 h-3 mt-0.5 rounded-full bg-green-500"
                    data-v-85662274=""
                  /><h3
                    class="text-lg"
                    data-v-85662274=""
                  >
                    <a
                      href="#"
                      class="font-semibold focus:outline-none hover:text-home-teal-button"
                      data-v-85662274=""
                    > Broken Links </a>
                  </h3>
                </div><div
                  class="mt-8"
                  data-v-85662274=""
                >
                  <p
                    class="mt-2 text-sm text-gray-500 dark:text-gray-400"
                    data-v-85662274=""
                  >
                    Your site has no broken links.
                  </p><!----><div data-v-85662274="">
                    <dl
                      class="grid grid-cols-1 mt-5 overflow-hidden bg-white border divide-y divide-gray-200 rounded-lg dark:bg-gray-600 dark:border-gray-500 dark:divide-gray-500 md:grid-cols-2 md:divide-y-0 md:divide-x"
                      data-v-85662274=""
                    >
                      <div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          Broken Links
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            0
                          </div>
                        </dd>
                      </div><div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          URLs Crawled
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            583
                          </div>
                        </dd>
                      </div>
                    </dl>
                  </div>
                </div>
              </div><div
                class="relative p-6 bg-white dark:bg-gray-700 group"
                data-v-85662274=""
              >
                <div
                  class="flex items-center space-x-2"
                  data-v-85662274=""
                >
                  <div
                    class="w-3 h-3 mt-0.5 rounded-full bg-green-500"
                    data-v-85662274=""
                  /><h3
                    class="text-lg"
                    data-v-85662274=""
                  >
                    <a
                      href="#"
                      class="font-semibold focus:outline-none hover:text-home-teal-button"
                      data-v-85662274=""
                    > Mixed Content </a>
                  </h3>
                </div><div
                  class="mt-8"
                  data-v-85662274=""
                >
                  <p
                    class="mt-2 text-sm text-gray-500 dark:text-gray-400"
                    data-v-85662274=""
                  >
                    We did not find mixed content. We last checked an hour ago.
                  </p><!----><div data-v-85662274="">
                    <dl
                      class="grid grid-cols-1 mt-5 overflow-hidden bg-white border divide-y divide-gray-200 rounded-lg dark:bg-gray-600 dark:divide-gray-500 dark:border-gray-500 md:grid-cols-2 md:divide-y-0 md:divide-x"
                      data-v-85662274=""
                    >
                      <div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          Pieces of Mixed Content
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            0
                          </div>
                        </dd>
                      </div><div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          URLs Crawled
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            0
                          </div>
                        </dd>
                      </div>
                    </dl>
                  </div>
                </div>
              </div><div
                class="relative p-6 bg-white dark:bg-gray-700 group"
                data-v-85662274=""
              >
                <div
                  class="flex items-center space-x-2"
                  data-v-85662274=""
                >
                  <div
                    class="w-3 h-3 mt-0.5 rounded-full bg-blue-500"
                    data-v-85662274=""
                  /><h3
                    class="text-lg"
                    data-v-85662274=""
                  >
                    <a
                      href="#"
                      class="font-semibold focus:outline-none hover:text-home-teal-button"
                      data-v-85662274=""
                    > Certificate transparency </a>
                  </h3>
                </div><div
                  class="mt-8"
                  data-v-85662274=""
                >
                  <p
                    class="mt-2 text-sm text-gray-500 dark:text-gray-400"
                    data-v-85662274=""
                  >
                    Your site is up. We last checked less than a minute ago.
                  </p>
                </div>
              </div><div
                class="relative p-6 bg-white dark:bg-gray-700 group"
                data-v-85662274=""
              >
                <div
                  class="flex items-center space-x-2"
                  data-v-85662274=""
                >
                  <div
                    class="w-3 h-3 mt-0.5 rounded-full bg-green-500"
                    data-v-85662274=""
                  /><h3
                    class="text-lg"
                    data-v-85662274=""
                  >
                    <a
                      href="#"
                      class="font-semibold focus:outline-none hover:text-home-teal-button"
                      data-v-85662274=""
                    > Application Health </a>
                  </h3>
                </div><div
                  class="mt-8"
                  data-v-85662274=""
                >
                  <p
                    class="mt-2 text-sm text-gray-500 dark:text-gray-400"
                    data-v-85662274=""
                  >
                    Your application seems to be healthy. We last checked 10 minutes ago.
                  </p><!----><div data-v-85662274="">
                    <dl
                      class="grid grid-cols-1 mt-5 overflow-hidden bg-white border divide-y divide-gray-200 rounded-lg dark:bg-gray-600 dark:divide-gray-500 dark:border-gray-500 md:grid-cols-2 md:divide-y-0 md:divide-x"
                      data-v-85662274=""
                    >
                      <div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          Total Health Checks
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            2
                          </div>
                        </dd>
                      </div><div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          Failed Health Checks
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            0
                          </div>
                        </dd>
                      </div>
                    </dl>
                  </div>
                </div>
              </div><div
                class="relative p-6 bg-white dark:bg-gray-700 group"
                data-v-85662274=""
              >
                <div
                  class="flex items-center space-x-2"
                  data-v-85662274=""
                >
                  <div
                    class="w-3 h-3 mt-0.5 rounded-full bg-green-500"
                    data-v-85662274=""
                  /><h3
                    class="text-lg"
                    data-v-85662274=""
                  >
                    <a
                      href="#"
                      class="font-semibold focus:outline-none hover:text-home-teal-button"
                      data-v-85662274=""
                    > DNS </a>
                  </h3>
                </div><!----><div
                  class="mt-8"
                  data-v-85662274=""
                >
                  <p
                    class="mt-2 text-sm text-gray-500 dark:text-gray-400"
                    data-v-85662274=""
                  >
                    DNS records found. We last checked an hour ago.
                  </p><div data-v-85662274="">
                    <dl
                      class="grid grid-cols-1 mt-5 overflow-hidden bg-white border divide-y divide-gray-200 rounded-lg dark:border-gray-500 dark:bg-gray-600 dark:divide-gray-500 lg md:grid-cols-2 md:divide-y-0 md:divide-x"
                      data-v-85662274=""
                    >
                      <div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          DNS Records
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            18
                          </div>
                        </dd>
                      </div><div
                        class="p-2 sm:p-3"
                        data-v-85662274=""
                      >
                        <dt
                          class="text-sm text-gray-500 dark:text-gray-200"
                          data-v-85662274=""
                        >
                          URLs Crawled
                        </dt><dd
                          class="flex items-baseline justify-between mt-1 md:block lg:flex"
                          data-v-85662274=""
                        >
                          <div
                            class="pt-4 text-sm font-semibold text-gray-900 dark:text-gray-100"
                            data-v-85662274=""
                          >
                            20
                          </div>
                        </dd>
                      </div>
                    </dl>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>
      </div>
    </div>
  </div>
</template>
