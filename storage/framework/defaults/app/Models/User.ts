...
export default defineModel({
  ...
  hasMany: [
    // Sanctum-shaped API tokens. The model maps `oauth_access_tokens`, which is
    // the table `@stacksjs/auth` already issues and checks tokens against - so
    // this relation lists exactly what `createToken()` minted, and revoking a
    // row here is the same revocation the auth layer reads.
    'PersonalAccessToken',
    'Customer',
    'TeamMember',
  ],
