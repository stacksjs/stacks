...
<template>
    <aside class="bg-black/2 lg:fixed lg:bottom-0 lg:right-0 lg:top-16 lg:w-96 lg:overflow-y-auto lg:border-l lg:border-white/5">
        <header class="flex items-center justify-between border-b border-white/5 px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
            <h2 class="text-base font-semibold leading-7 dark:text-white">Activity feed</h2>
        </header>
        <ul v-if="githubStore.hasCommits" role="list" class="divide-y divide-gray-200">
            <li v-for="(commit, index) in githubStore.getCommits" :key="index" class="px-4 py-4 sm:px-6 lg:px-8">
                ...
                <p class="mt-3 text-sm text-gray-500"><a :href="commit.commit.url" class="font-mono hover:text-gray-500 text-gray-400 truncate">{{ commit.sha.slice(0, 10) }}</a> on <span class="text-gray-400">main</span></p>
            </li>
        </ul>
    </aside>
</template>
