...
export interface DashboardOptions {
   ...
   * Errors and Analytics sections each resolve one provider, so a project can
   * point Logs at a hosted product while Analytics stays local. Every domain
   * defaults to `'local'`, which is the framework's own behaviour: the `logs`
   * table, the error store, and the recorded `Request` rows in this app's
   * database.
   *
   * Selection is per domain on purpose. There is no global switch, because
   * these are four independent products and an application usually adopts them
   * one at a time.
   *
   * A provider only ever reads. Nothing in this section writes to a remote
   * product, and the section's own write paths (resolving or ignoring an
   * error, for instance) always stay local.
   *
   * @example
   * providers: {
   *   logs: { driver: 'hq', drivers: { hq: { baseUrl: 'https://loghq.org', token: env.LOGHQ_READ_TOKEN } } },
   *   errors: { driver: 'local' },
   * },
   *
   * @default all domains 'local'
   */
  providers?: DashboardProviderOptions
}

/**
 * The backends a dashboard section can read from.
 *
 * `local` reads this application's own database and is the default.
 * `hq` reads the matching hosted product (loghq, bughq, analyticshq).
 */
export type DashboardProviderName = 'local' | 'hq'

/**
 * Credentials and endpoint for a hosted HQ product.
 *
 * `token` must be a READ token. A product's ingest key (the `loghq_...` value
 * an app uses to SEND logs) grants no read access at all, so pasting one here
 * produces an unauthorized response rather than data. The hosted read APIs
 * currently authenticate a user rather than a machine, which is why this is
 * separate from whatever ingest key `config/logging.ts` carries.
 */
export interface DashboardHqDriverOptions {
  /** Product API origin, e.g. `https://loghq.org`. */
  baseUrl?: string
  /** Read token. Not an ingest key. See the note on this interface. */
  token?: string
  /** Project identifier within the product. */
  project?: string
  /** Request timeout in milliseconds. Defaults to 5000. */
  timeoutMs?: number
}

/** One domain's provider choice, plus per-driver settings. */
export interface DashboardProviderSelection {
  /** Which backend this domain reads from. Defaults to `'local'`. */
  driver?: DashboardProviderName
  /** Settings for the non-local drivers. Ignored while `driver` is `'local'`. */
  drivers?: {
    hq?: DashboardHqDriverOptions
  }
}

/** Per-domain provider selection. Omit a domain to leave it local. */
export interface DashboardProviderOptions {
  /** The Logs section. Local reads the `logs` table. */
  logs?: DashboardProviderSelection
  /** The Errors section. Local reads this application's error store. */
  errors?: DashboardProviderSelection
  /** The web Analytics section. Local aggregates recorded `Request` rows. */
  analytics?: DashboardProviderSelection
}
