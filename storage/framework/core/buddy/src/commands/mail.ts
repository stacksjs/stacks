...
export function mailCommands(buddy: CLI): void {
  ...
  buddy
    ...
    .action(async (options: { port?: string, uiPort?: string, open?: boolean }) => {
      loadEnvFiles()
      await runLocalMailCatcher({
        smtpPort: Number(options.port) || LOCAL_MAIL_SMTP_PORT,
        uiPort: Number(options.uiPort) || LOCAL_MAIL_UI_PORT,
...
/**
 * The local catcher's ports, which are mailpit's ports.
 *
 * Deliberately not the mail server's own defaults (2525/8080). Everything
 * anybody has ever pointed at a development mail trap — a README, a
 * docker-compose port mapping, a bookmarked inbox — is pointed at 1025 and
 * 8025, and adopting those numbers means adopting this costs nobody a
 * configuration change. `.env.example` and `config/services.ts` agree.
 */
export const LOCAL_MAIL_SMTP_PORT = 1025
export const LOCAL_MAIL_UI_PORT = 8025

/** Where the catcher keeps its mailboxes, under the project rather than in $HOME. */
export const LOCAL_MAIL_DIR = 'storage/framework/mail'

/**
 * Find the `mail` binary.
 *
 * pantry installs it project-locally (`pantry/.bin`), so a globally-installed
 * copy is checked last rather than first: a project that pinned a version must
 * get the version it pinned, not whatever happens to be on the developer's
 * PATH.
 */
export function resolveMailBinary(root: string = process.cwd()): string | undefined {
  const local = `${root}/pantry/.bin/mail`
  if (existsSync(local))
    return local
  ...
  try {
    ...
    /* Nothing on PATH — fall through to the install hint below. */
  }
 ...
 *
 * The same program production runs, with delivery off, authentication off, and
 * bound to loopback. That last one is not tidiness: a process that accepts every
 * message for every recipient and shows them in a UI with no password is an open
 * relay and an open inbox at once, and it must never be reachable from the
 * network.
 */
export function buildLocalMailConfig(options: { smtpPort: number }): string {
  return [
    '# Generated by `buddy mail:dev`. Edits are overwritten on the next run.',
    '',
    '[server]',
    'host = "127.0.0.1"',
    ...
    '',
    '[antispam]',
    // A trap that scores its own test mail as spam and files it into Junk is a
    // trap where the message you were looking for is not in the inbox.
    'spam_filter = false',
    'dnsbl = false',
    'greylist = false',
    '',
...
async function runLocalMailCatcher(options: { smtpPort: number, uiPort: number, open: boolean }): Promise<void> {
  ...
  if (options.open) {
    try {
      execSync(`open "http://localhost:${options.uiPort}" 2>/dev/null || xdg-open "http://localhost:${options.uiPort}" 2>/dev/null || true`, { encoding: 'utf-8' })
    }
    catch {
      /* Silent — the URL is printed above, which is enough headless. */
    }
  }
  ...
  const child = Bun.spawn([binary, 'serve', '--config', configPath], {
    ...
    env: {
      ...process.env,
      SMTP_DB_PATH: `${dir}/catcher.db`,
      SMTP_ENABLE_WEBMAIL: 'true',
      SMTP_WEBMAIL_PORT: String(options.uiPort),
      // A Secure cookie is dropped by the browser over plain-HTTP loopback, so
      // the UI would take a login and then behave as though nobody had logged in.
      SMTP_WEBMAIL_SECURE_COOKIES: 'false',
    },
    stdout: 'inherit',
    stderr: 'inherit',
  })
