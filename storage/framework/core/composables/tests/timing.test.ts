...
describe('useDebouncedRef', () => {
  ...
  it('should debounce value updates', async () => {
    const r = useDebouncedRef('initial', 50)
    ...
    await sleep(80)
    expect(r.value).toBe('updated')
  })

  it('should use the latest value when debounced', async () => {
    const r = useDebouncedRef(0, 50)
  ...
  it('should use default delay of 200ms', async () => {
    ...
    await sleep(150)
    expect(r.value).toBe('changed')
  })

  it('should handle rapid updates', async () => {
    const r = useDebouncedRef(0, 30)
  ...
  it('should work with object values', async () => {
    const r = useDebouncedRef({ count: 0 }, 50)
  ...
  it('should handle boolean values', async () => {
    const r = useDebouncedRef(false, 50)
  ...
  it('should reset debounce timer on each update', async () => {
    const r = useDebouncedRef('a', 60)

    r.value = 'b'
    await sleep(30)
    r.value = 'c'
    await sleep(30)
    // Timer was reset, so 'c' should not have propagated yet
  ...
  it('should allow multiple sequential debounced updates', async () => {
    const r = useDebouncedRef(0, 30)
  ...
  it('should handle null and undefined', async () => {
    const r = useDebouncedRef<string | null>('hello', 50)
...
describe('useThrottledRef', () => {
  ...
  it('should propagate the first update immediately', async () => {
    const r = useThrottledRef(0, 100)

    r.value = 1
    // The watch is async, so we need a tiny delay for reactivity to propagate
    await sleep(10)
    expect(r.value).toBe(1)
  })

  it('should throttle subsequent rapid updates', async () => {
    const r = useThrottledRef(0, 100)

    r.value = 1
    await sleep(10) // let first update propagate
    expect(r.value).toBe(1)
    ...
    // After throttle period, trailing value should apply
    await sleep(120)
    expect(r.value).toBe(4)
  })

  it('should use default delay of 200ms', async () => {
    const r = useThrottledRef(0)
  ...
  it('should handle object values', async () => {
    const r = useThrottledRef({ n: 0 }, 50)
  ...
  it('should handle boolean values', async () => {
    const r = useThrottledRef(false, 50)
  ...
  it('should handle null values', async () => {
    const r = useThrottledRef<string | null>('test', 50)
  ...
  it('should handle multiple sequential throttle periods', async () => {
    const r = useThrottledRef(0, 40)

    r.value = 1
    await sleep(10)
    expect(r.value).toBe(1)

    await sleep(50) // wait for throttle to clear
