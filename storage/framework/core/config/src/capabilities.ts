...
export const capabilityRegistry: readonly CapabilityDriver[] = [
  ...
  { category: 'storage', name: 'azure', status: 'partial', implementation: 'storage/framework/core/storage/src/adapters/azure.ts', testEvidence: ['storage/framework/core/storage/tests/azure-adapter.test.ts', 'storage/framework/core/storage/tests/azure-signing.test.ts'], topology: 'managed-object-storage', prerequisites: ['An Azure storage account, a container, and an account key or SAS token'], limitations: ['CI covers Shared Key and service-SAS signing plus request construction, not a live container round trip.', 'Visibility is a container property on Azure, so changeVisibility() refuses rather than silently doing nothing; a per-object grant is signedUrl().', 'ensureConfiguredBuckets provisions S3 buckets only, so the container must exist before the disk is used.'] },
