...
// Runtime config primitives shared by the broad package entrypoints and narrow consumers.
...
export const config: StacksOptions = new Proxy(proxyTarget, {
  ...
  set(_t, prop: string, value: unknown) {
    ;(overrides as unknown as Record<string | symbol, unknown>)[prop] = value

    return true
  },
  deleteProperty(_t, prop: string) {
    delete (overrides as unknown as Record<string | symbol, unknown>)[prop]
...
export let featureFlags: StacksOptions['featureFlags'] = config.featureFlags
...
export let server: StacksOptions['server'] = config.server
