...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} two-factor enable and disable verify persisted state`, async () => {
    ...
    const directory = await mkdtemp(join(tmpdir(), 'stacks-two-factor-state-'))
    const name = `stacks_two_factor_state_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/two-factor-state-atomicity.ts`], {
        env: {
          ...
          STACKS_TWO_FACTOR_STATE_CONFIG: config,
      ...
      try {
        ...
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('two-factor state atomicity OK')
      }
