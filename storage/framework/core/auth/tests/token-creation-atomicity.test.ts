...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} token creation is atomic and preserves transaction scope`, async () => {
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/token-creation-atomicity.ts`], {
        env: {
          ...
          STACKS_TOKEN_CREATION_CONFIG: config,
      ...
      try {
        ...
        expect(timedOut, 'token creation must finish without a watchdog kill').toBe(false)
        expect(code, `${stdout}\n${stderr}`).toBe(0)
