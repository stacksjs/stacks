...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} authentication credentials are consumed atomically`, async () => {
    const url = dialect === 'sqlite' ? undefined : new URL(connection!)
    if (url && !['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname))
      throw new Error('Credential tests require a local disposable database server')
    const directory = await mkdtemp(join(tmpdir(), 'stacks-credential-test-'))
    const name = `stacks_credential_test_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/credential-consumption.ts`], {
        cwd: directory,
        env: {
          ...process.env, APP_ENV: 'test', DB_CONNECTION: dialect, DB_QUERY_LOGGING_ENABLED: 'false',
          DB_DATABASE_PATH: dialect === 'sqlite' ? join(directory, 'credentials.sqlite') : ':memory:',
          STACKS_CREDENTIAL_TEST_CONFIG: config,
      ...
      try {
        ...
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('credential consumption OK')
      }
