...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} refresh rejects a revoked OAuth client`, async () => {
    const url = dialect === 'sqlite' ? undefined : new URL(connection!)
    if (url && !['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname))
      throw new Error('Token refresh tests require a local disposable database server')
    const directory = await mkdtemp(join(tmpdir(), 'stacks-token-refresh-'))
    const name = `stacks_token_refresh_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/token-refresh-client.ts`], {
        env: {
          ...
          STACKS_TOKEN_REFRESH_CONFIG: config,
      ...
      try {
        ...
        expect(timedOut, 'token refresh must finish without a watchdog kill').toBe(false)
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('token refresh client validity OK')
      }
