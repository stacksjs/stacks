...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} magic-link replacement and delivery are atomic`, async () => {
    ...
    if (url && (!['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname) || !url.port || ['5432', '3306'].includes(url.port)))
      throw new Error('Magic-link tests require a disposable local server on a non-default port')
    const directory = await mkdtemp(join(tmpdir(), 'stacks-magic-send-'))
    const name = `stacks_magic_send_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/magic-link-send-atomicity.ts`], {
        env: {
          ...
          STACKS_MAGIC_SEND_CONFIG: config,
      ...
      try {
        ...
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('magic-link send atomicity OK')
      }
