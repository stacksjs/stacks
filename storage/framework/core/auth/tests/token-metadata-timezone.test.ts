...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} token metadata retains UTC instants across timezones`, async () => {
    ...
    if (url && (!['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname) || !url.port || ['5432', '3306'].includes(url.port)))
      throw new Error('Token metadata tests require a disposable local server on a non-default port')
    const directory = await mkdtemp(join(tmpdir(), 'stacks-token-metadata-'))
    const name = `stacks_token_metadata_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/token-metadata-timezone.ts`], {
        env: {
          ...
          STACKS_TOKEN_METADATA_CONFIG: config,
      ...
      try {
        ...
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('token metadata timezone OK')
      }
