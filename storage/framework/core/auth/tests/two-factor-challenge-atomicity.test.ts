...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} two-factor login challenge replacement is atomic`, async () => {
    ...
    const directory = await mkdtemp(join(tmpdir(), 'stacks-two-factor-challenge-'))
    const name = `stacks_two_factor_challenge_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/two-factor-challenge-atomicity.ts`], {
        env: {
          ...
          STACKS_TWO_FACTOR_CHALLENGE_CONFIG: config,
      ...
      try {
        ...
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('two-factor challenge atomicity OK')
      }
