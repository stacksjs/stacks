...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} verification resend cooldown is claimed once`, async () => {
    ...
    const directory = await mkdtemp(join(tmpdir(), 'stacks-email-resend-'))
    const name = `stacks_email_resend_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/email-verification-resend-race.ts`], {
        env: {
          ...
          STACKS_EMAIL_RESEND_CONFIG: config,
      ...
      try {
        ...
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('verification resend race OK')
      }
