...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} token prune helpers return the rows they removed`, async () => {
    const url = dialect === 'sqlite' ? undefined : new URL(connection!)
    if (url && !['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname))
      throw new Error('Token prune count tests require a local disposable database server')
    const directory = await mkdtemp(join(tmpdir(), 'stacks-token-prune-counts-'))
    const name = `stacks_token_prune_counts_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/token-prune-counts.ts`], {
        env: {
          ...process.env, APP_ENV: 'test', DB_CONNECTION: dialect, DB_QUERY_LOGGING_ENABLED: 'false',
          DB_DATABASE_PATH: dialect === 'sqlite' ? join(directory, 'prune-counts.sqlite') : ':memory:',
          STACKS_TOKEN_PRUNE_COUNTS_CONFIG: config,
      ...
      try {
        ...
        expect(timedOut, 'token prune counts must finish without a watchdog kill').toBe(false)
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('token prune counts OK')
      }
