...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} pending two-factor setup replacement is atomic`, async () => {
    ...
    const directory = await mkdtemp(join(tmpdir(), 'stacks-two-factor-setup-'))
    const name = `stacks_two_factor_setup_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/two-factor-setup-atomicity.ts`], {
        env: {
          ...
          STACKS_TWO_FACTOR_SETUP_CONFIG: config,
      ...
      try {
        ...
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('two-factor setup atomicity OK')
      }
