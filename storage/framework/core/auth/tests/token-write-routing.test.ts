...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} token mutations preserve request read-your-writes`, async () => {
    ...
    const directory = await mkdtemp(join(tmpdir(), 'stacks-token-routing-'))
    const name = `stacks_token_routing_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/token-write-routing.ts`], {
        env: {
          ...
          STACKS_TOKEN_ROUTING_CONFIG: config,
      ...
      try {
        ...
        expect(timedOut, 'token routing must finish without a watchdog kill').toBe(false)
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('token write routing OK')
      }
