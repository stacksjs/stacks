...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} prunable returns the rows it deleted`, async () => {
    const url = dialect === 'sqlite' ? undefined : new URL(connection!)
    if (url && !['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname))
      throw new Error('ORM prune count tests require a local disposable database server')
    const directory = await mkdtemp(join(tmpdir(), 'stacks-orm-prune-counts-'))
    const name = `stacks_orm_prune_counts_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/prune-counts.ts`], {
        env: {
          ...
          STACKS_ORM_PRUNE_COUNTS_CONFIG: config,
      ...
      try {
        ...
        expect(timedOut, 'orm prune counts must finish without a watchdog kill').toBe(false)
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('orm prune counts OK')
      }
