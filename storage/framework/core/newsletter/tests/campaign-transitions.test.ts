...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} campaign delivery transitions run`, async () => {
    const url = dialect === 'sqlite' ? undefined : new URL(connection!)
    if (url && !['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname))
      throw new Error('Campaign transition tests require a local disposable database server')
    const directory = await mkdtemp(join(tmpdir(), 'stacks-campaign-transitions-'))
    const name = `stacks_campaign_transitions_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/campaign-transitions.ts`], {
        env: {
          ...process.env, APP_ENV: 'test', DB_CONNECTION: dialect, DB_QUERY_LOGGING_ENABLED: 'false',
          DB_DATABASE_PATH: dialect === 'sqlite' ? join(directory, 'campaign-transitions.sqlite') : ':memory:',
          STACKS_CAMPAIGN_TRANSITIONS_CONFIG: config,
      ...
      try {
        ...
        expect(timedOut, 'campaign transitions must finish without a watchdog kill').toBe(false)
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('campaign transitions OK')
      }
