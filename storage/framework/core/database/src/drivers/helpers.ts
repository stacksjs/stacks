/**
 * Driver-agnostic helpers used by `./mysql.ts`, `./postgres.ts`,
 * `./sqlite.ts`, and `./defaults/*`.
 *
 * Why these live here and not in `./index.ts`:
 *
 * `./index.ts` re-exports every driver (`export * from './sqlite'`, etc.).
 * If the helpers were defined in `./index.ts`, then any driver module
 * importing a helper would do `import { ... } from '.'`, which would
 * load `./index.ts`, which would `export * from './sqlite'` — back to
 * the same driver module that's still mid-evaluation. Bun's ESM loader
 * can technically handle this via live bindings, but in practice the
 * graph deadlocks (60s @ 99% CPU) when @stacksjs/database is imported
 * outside the framework's preloader context (e.g. by `bun test`'s
 * preload chain).
 *
 * Putting the helpers in a leaf file with no driver re-exports breaks
 * the cycle: drivers import from `./helpers`, and the helpers themselves
 * never reach back into the drivers, so the load graph is acyclic.
 */
...
export async function getExecutedMigrations(): Promise<{ name: string }[]> {
  try {
    ...
    // `(db)` that would have unchecked the whole handle.
