...
for (const dialect of ['sqlite', 'postgres', 'mysql'] as const) {
  ...
  test.skipIf(dialect !== 'sqlite' && !connection)(`${dialect} transaction facade is atomic and preserves transaction scope`, async () => {
    const url = dialect === 'sqlite' ? undefined : new URL(connection!)
    if (url && !['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname))
      throw new Error('Transaction facade tests require a local disposable database server')
    const directory = await mkdtemp(join(tmpdir(), 'stacks-transaction-facade-'))
    const name = `stacks_transaction_facade_${crypto.randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      const child = Bun.spawn([process.execPath, `--config=${config}`, '--no-env-file', `${import.meta.dir}/fixtures/transaction-facade.ts`], {
        env: {
          ...process.env, APP_ENV: 'test', DB_CONNECTION: dialect, DB_QUERY_LOGGING_ENABLED: 'false',
          DB_DATABASE_PATH: dialect === 'sqlite' ? join(directory, 'scope.sqlite') : ':memory:',
          STACKS_TRANSACTION_FACADE_CONFIG: config,
      ...
      try {
        ...
        expect(timedOut, 'transaction facade must finish without a watchdog kill').toBe(false)
        expect(code, `${stdout}\n${stderr}`).toBe(0)
        expect(stdout).toContain('transaction facade atomicity OK')
      }
