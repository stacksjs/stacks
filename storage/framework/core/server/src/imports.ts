...
function scanDirTopLevel(dir: string): string[] {
  try {
    // Sorted, like every other scan here: see `sortedGlob`.
    return globSync(`${dir}/*.ts`, { ignore: ['**/*.d.ts', '**/index.ts', '**/README*'] }).sort()
  }
  catch {
    return []
  }
...
function scanDefineModelExports(dir: string, opts: { recursive?: boolean } = {}): ExportInfo[] {
  ...
  try {
    ...
    files = globSync(pattern, { ignore: ['**/*.d.ts', '**/index.ts', '**/README*'] }).sort()
  }
  catch {
    return []
  }
...
/*
 * Every directory scan below is sorted before it is emitted.
 *
 * `globSync` returns entries in FILESYSTEM order, and that order differs
 * between filesystems — macOS APFS and Linux ext4 do not agree. So the same
 * source tree generated a different barrel on a developer's machine than in
 * CI: same exports, different line order.
 *
 * The cost is not cosmetic. It means regenerating on another machine produces
 * a diff that looks like staleness, two developers regenerating fight over
 * these files for no reason, and no freshness check can ever be written —
 * which is what stacksjs/stacks#2408 ran into. Sorting makes the output a
 * function of the source rather than of the machine.
 */
...
`
  ...
  if (options.declarations !== false) {
    ...
    // The browser declaration is emptied rather than written. Nothing injects a
    // bare name into a template: stx attaches its own globals to `window` and
    // ships `stx.d.ts` for them, and no build step rewrites anything else into
    // an import (stacksjs/stacks#2585).
    const pruned = await pruneBrowserAutoImportTypes()
    if (pruned.length > 0)
      log.debug(`[auto-imports] dropped ${pruned.length} browser globals that nothing injects`)
...
export async function pruneBrowserAutoImportTypes(): Promise<string[]> {
  ...
  for (const line of source.split('\n')) {
    ...
    const [, declaredAs] = match

    /*
     * Every one of them goes, whichever module it names.
     *
     * The stx globals were always stx's to declare: it ships `stx.d.ts` and
     * references it from its own types entry, so TypeScript picks those up on
     * install. What took longer to see is that the rest are not ambient at all.
     * This file used to keep a declaration when the module beside it really did
     * export the name, on the reading that the stx plugin injected it. It does
     * not. Nothing does - `unplugin-auto-import` has not run here in a long
     * time, and neither `bun-plugin-stx` nor `@stacksjs/stx` rewrites a bare
     * name into an import.
     *
     * Measured against the runtime stx actually serves: 83 names declared here,
     * 61 attached to `window`, three in both. So 80 of them typechecked and then
     * raised a `ReferenceError` during setup, which does not degrade the one
     * call - it takes the whole root down unhydrated. Five framework components
     * were doing exactly that (stacksjs/stacks#2585).
     *
     * A name that is genuinely reachable is reachable through stx's own
     * declarations; anything else wants an import, and `tsc` should say so.
     */
  ...
  // thought to check it.
  // Both spellings of the old header, because this file has carried two: the
  // original provenance line, and the one written when the prune still kept
  // whatever the module beside it exported. Neither describes what is left.
  const staleHeaders = new Set([
    '// Generated by unplugin-auto-import',
    '// Pruned by Stacks: every name below is exported by the module beside it.',
  ])
  const pruned = kept
    .filter(line => line.trim() !== '// @ts-nocheck')
    .map(line => staleHeaders.has(line.trim())
      ? '// Emptied by Stacks: nothing injects a bare name into a template. See stacksjs/stacks#2585.'
      : line)
    .join('\n')

  // Compared against the file rather than against `removed`, so a run with
  // nothing left to drop still corrects a stale header. Keyed on the content,
  // so it stays a no-op once both agree.
  if (pruned === source)
    return removed
