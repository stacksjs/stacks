...
export class DeploymentModel {
  ...
  static async create(newDeployment: NewDeployment): Promise<DeploymentModel> {
    const model = await db.insertInto('deployments').values(newDeployment).returningAll().executeTakeFirstOrThrow()
  ...
  static async remove(id: number): Promise<DeploymentModel> {
    const model = await db.deleteFrom('deployments').where('id', '=', id).returningAll().executeTakeFirstOrThrow()
  ...
  async orderBy(column: keyof DeploymentType, order: 'asc' | 'desc') {
    return await db.selectFrom('deployments').selectAll().orderBy(column, order).execute()
  }

  async orderByDesc(column: keyof DeploymentType) {
    return await db.selectFrom('deployments').selectAll().orderBy(column, 'desc').execute()
  }

  async orderByAsc(column: keyof DeploymentType) {
    return await db.selectFrom('deployments').selectAll().orderBy(column, 'asc').execute()
  }
  ...
  async update(deployment: DeploymentUpdate): Promise<Result<DeploymentType, Error>> {
    if (this.deployment.id === undefined) return err(handleError('Deployment ID is undefined'))
    ...
    if (!updatedModel) return err(handleError('Deployment not found'))
  ...
  async delete(): Promise<void> {
    if (this.deployment.id === undefined) throw new Error('Deployment ID is undefined')

    await db.deleteFrom('deployments').where('id', '=', this.deployment.id).execute()
  ...
  async refresh(): Promise<void> {
    ...
    if (!refreshedModel) throw new Error('Deployment not found')
...
export async function count() {
  const results = await db.selectFrom('deployments').selectAll().execute()
...
export async function get(
  ...
  sort: { column: keyof DeploymentType; order: 'asc' | 'desc' } = { column: 'created_at', order: 'desc' },
) {
  let query = db.selectFrom('deployments')
...
export async function create(newDeployment: NewDeployment) {
  return await db.insertInto('deployments').values(newDeployment).returningAll().executeTakeFirstOrThrow()
}
...
export async function recent(limit: number) {
  return await db.selectFrom('deployments').selectAll().limit(limit).execute()
}

export async function last(limit: number) {
  return await db.selectFrom('deployments').selectAll().orderBy('id', 'desc').limit(limit).execute()
}

export async function update(id: number, deploymentUpdate: DeploymentUpdate) {
  return await db.updateTable('deployments').set(deploymentUpdate).where('id', '=', id).execute()
}

export async function remove(id: number) {
  return await db.deleteFrom('deployments').where('id', '=', id).returningAll().executeTakeFirst()
}
...
export async function whereIs(criteria: Partial<DeploymentType>, options: QueryOptions = {}) {
  let query = db.selectFrom('deployments')
...
export async function whereIn(column: keyof DeploymentType, values: any[], options: QueryOptions = {}) {
  let query = db.selectFrom('deployments')
