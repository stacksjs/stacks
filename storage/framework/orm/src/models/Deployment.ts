...
export class DeploymentModel {
  ...
  static async remove(id: number): Promise<DeploymentModel> {
    const model = await db.deleteFrom('deployments').where('id', '=', id).executeTakeFirstOrThrow()
...
export async function all(limit = 10, offset = 0): Promise<DeploymentType[]> {
  return await db
    .selectFrom('deployments')
    .selectAll()
...
export async function remove(id: number) {
  return await db.deleteFrom('deployments').where('id', '=', id).executeTakeFirst()
}
