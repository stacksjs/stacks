...
export class ProductReviewModel extends BaseOrm<ProductReviewModel> {
  ...
  applyWhere<V>(column: keyof ProductReviewsTable, ...args: [V] | [Operator, V]): ProductReviewModel {
    if (args.length === 1) {
      const [value] = args
  ...
  where<V = string>(column: keyof ProductReviewsTable, ...args: [V] | [Operator, V]): ProductReviewModel {
