...
export class PostModel {
  ...
  static async create(newPost: NewPost): Promise<PostModel> {
    const model = await db.insertInto('posts').values(newPost).returningAll().executeTakeFirstOrThrow()
  ...
  static async remove(id: number): Promise<PostModel> {
    const model = await db.deleteFrom('posts').where('id', '=', id).returningAll().executeTakeFirstOrThrow()
...
export async function create(newPost: NewPost) {
  return await db.insertInto('posts').values(newPost).returningAll().executeTakeFirstOrThrow()
}
...
export async function remove(id: number) {
  return await db.deleteFrom('posts').where('id', '=', id).returningAll().executeTakeFirst()
}
