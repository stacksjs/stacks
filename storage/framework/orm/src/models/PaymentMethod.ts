   ...
   this.provider_id = paymentmethod?.provider_id
    ...
    this.user_id = paymentmethod?.user_id
      ...
      async find(id: number): Promise<PaymentMethodModel | undefined> {
        let query = db.selectFrom('payment_methods').where('id', '=', id).selectAll()
      ...
      static async paginate(options: QueryOptions = { limit: 10, offset: 0, page: 1 }): Promise<PaymentMethodResponse> {
        ...
        const payment_methodsWithExtra = await db.selectFrom('payment_methods')
          ...
          let nextCursor = null
          if (payment_methodsWithExtra.length > (options.limit ?? 10)) nextCursor = payment_methodsWithExtra.pop()?.id ?? null
      ...
      whereNull(column: string): PaymentMethodModel {
       ...
       static whereName(value: string): PaymentMethodModel {
        const instance = new PaymentMethodModel(null)
...
static whereDescription(value: string): PaymentMethodModel {
        const instance = new PaymentMethodModel(null)
...
static whereAmount(value: string): PaymentMethodModel {
        const instance = new PaymentMethodModel(null)
...
name: this.name,
   ...
   provider_id: this.provider_id,
    ...
    export async function whereName(value: string): Promise<PaymentMethodModel[]> {
        const query = db.selectFrom('payment_methods').where('name', '=', value)
        const results = await query.execute()

        return results.map(modelItem => new PaymentMethodModel(modelItem))
      } 

export async function whereDescription(value: string): Promise<PaymentMethodModel[]> {
        const query = db.selectFrom('payment_methods').where('description', '=', value)
        const results = await query.execute()

        return results.map(modelItem => new PaymentMethodModel(modelItem))
      } 

export async function whereAmount(value: number): Promise<PaymentMethodModel[]> {
        const query = db.selectFrom('payment_methods').where('amount', '=', value)
        const results = await query.execute()
...
export async function whereProviderId(value: string): Promise<PaymentMethodModel[]> {
    ...
    m => new PaymentMethodModel(modelItem))
      } 
