...
export class ReleaseModel {
  ...
  static async remove(id: number): Promise<ReleaseModel> {
    const model = await db.deleteFrom('releases').where('id', '=', id).executeTakeFirstOrThrow()
  ...
  async orderBy(column: keyof ReleaseType, order: 'asc' | 'desc'): Promise<ReleaseType[]> {
    return await db.selectFrom('releases').selectAll().orderBy(column, order).execute()
  }

  async orderByDesc(column: keyof ReleaseType): Promise<ReleaseType[]> {
    return await db.selectFrom('releases').selectAll().orderBy(column, 'desc').execute()
  }

  async orderByAsc(column: keyof ReleaseType): Promise<ReleaseType[]> {
    return await db.selectFrom('releases').selectAll().orderBy(column, 'asc').execute()
  }
  ...
  async update(release: ReleaseUpdate): Promise<Result<ReleaseType, Error>> {
    if (this.release.id === undefined) return err(handleError('Release ID is undefined'))
    ...
    if (!updatedModel) return err(handleError('Release not found'))
  ...
  async delete(): Promise<void> {
    if (this.release.id === undefined) throw new Error('Release ID is undefined')

    await db.deleteFrom('releases').where('id', '=', this.release.id).execute()
  ...
  async refresh(): Promise<void> {
    ...
    if (!refreshedModel) throw new Error('Release not found')
...
export async function count(): Number {
  const results = await db.selectFrom('releases').selectAll().execute()
...
export async function get(
  ...
  sort: { column: keyof ReleaseType; order: 'asc' | 'desc' } = { column: 'created_at', order: 'desc' },
) {
  let query = db.selectFrom('releases')
...
export async function all(limit = 10, offset = 0): Promise<ReleaseType[]> {
  return await db.selectFrom('releases').selectAll().orderBy('created_at', 'desc').limit(limit).offset(offset).execute()
}
...
export async function recent(limit: number): Promise<ReleaseModel[]> {
  return await db.selectFrom('releases').selectAll().limit(limit).execute()
}

export async function last(limit: number): Promise<ReleaseType> {
  return await db.selectFrom('releases').selectAll().orderBy('id', 'desc').limit(limit).execute()
}

export async function update(id: number, releaseUpdate: ReleaseUpdate) {
  return await db.updateTable('releases').set(releaseUpdate).where('id', '=', id).execute()
}

export async function remove(id: number) {
  return await db.deleteFrom('releases').where('id', '=', id).executeTakeFirst()
}
...
export async function whereIs(criteria: Partial<ReleaseType>, options: QueryOptions = {}) {
  let query = db.selectFrom('releases')
...
export async function whereIn(column: keyof ReleaseType, values: any[], options: QueryOptions = {}) {
  let query = db.selectFrom('releases')
