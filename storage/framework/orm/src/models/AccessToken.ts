...
export class AccessTokenModel {
  ...
  static async all(options: QueryOptions = { limit: 10, offset: 0, page: 1 }): Promise<AccessTokenResponse> {
    ...
    const access_tokensWithExtra = await db
      .selectFrom('access_tokens')
      .selectAll()
    ...
    let nextCursor = null
    if (access_tokensWithExtra.length > (options.limit ?? 10)) nextCursor = access_tokensWithExtra.pop()?.id // Use the ID of the extra record as the next cursor
  ...
  static async create(newAccessToken: NewAccessToken): Promise<AccessTokenModel> {
    const model = await db.insertInto('access_tokens').values(newAccessToken).returningAll().executeTakeFirstOrThrow()
  ...
  static async remove(id: number): Promise<AccessTokenModel> {
    const model = await db.deleteFrom('access_tokens').where('id', '=', id).returningAll().executeTakeFirstOrThrow()
  ...
  async first() {
    return await db.selectFrom('access_tokens').selectAll().executeTakeFirst()
  }

  async last() {
    return await db.selectFrom('access_tokens').selectAll().orderBy('id', 'desc').executeTakeFirst()
  }

  async orderBy(column: keyof AccessTokenType, order: 'asc' | 'desc') {
    return await db.selectFrom('access_tokens').selectAll().orderBy(column, order).execute()
  }

  async orderByDesc(column: keyof AccessTokenType) {
    return await db.selectFrom('access_tokens').selectAll().orderBy(column, 'desc').execute()
  }

  async orderByAsc(column: keyof AccessTokenType) {
    return await db.selectFrom('access_tokens').selectAll().orderBy(column, 'asc').execute()
  }
  ...
  async update(accesstoken: AccessTokenUpdate): Promise<Result<AccessTokenType, Error>> {
    if (this.accesstoken.id === undefined) return err(handleError('AccessToken ID is undefined'))
    ...
    if (!updatedModel) return err(handleError('AccessToken not found'))
  ...
  async delete(): Promise<void> {
    if (this.accesstoken.id === undefined) throw new Error('AccessToken ID is undefined')

    await db.deleteFrom('access_tokens').where('id', '=', this.accesstoken.id).execute()
  ...
  async refresh(): Promise<void> {
    ...
    if (!refreshedModel) throw new Error('AccessToken not found')

    this.accesstoken = refreshedModel
  }

  async team() {
    if (this.accesstoken.id === undefined) throw new Error('Relation Error!')
...
export async function count() {
  const results = await db.selectFrom('access_tokens').selectAll().execute()
...
export async function get(
  ...
  sort: { column: keyof AccessTokenType; order: 'asc' | 'desc' } = { column: 'created_at', order: 'desc' },
) {
  let query = db.selectFrom('access_tokens')
...
export async function create(newAccessToken: NewAccessToken) {
  return await db.insertInto('access_tokens').values(newAccessToken).returningAll().executeTakeFirstOrThrow()
}
...
export async function recent(limit: number) {
  return await db.selectFrom('access_tokens').selectAll().limit(limit).execute()
}

export async function last(limit: number) {
  return await db.selectFrom('access_tokens').selectAll().orderBy('id', 'desc').limit(limit).execute()
}

export async function update(id: number, accesstokenUpdate: AccessTokenUpdate) {
  return await db.updateTable('access_tokens').set(accesstokenUpdate).where('id', '=', id).execute()
}

export async function remove(id: number) {
  return await db.deleteFrom('access_tokens').where('id', '=', id).returningAll().executeTakeFirst()
}
...
export async function whereIs(criteria: Partial<AccessTokenType>, options: QueryOptions = {}) {
  let query = db.selectFrom('access_tokens')
...
export async function whereIn(column: keyof AccessTokenType, values: any[], options: QueryOptions = {}) {
  let query = db.selectFrom('access_tokens')
