...
export class AccessTokenModel {
  ...
  static async create(newAccessToken: NewAccessToken): Promise<AccessTokenModel> {
    const result = await db.insertInto('access_tokens').values(newAccessToken).executeTakeFirstOrThrow()
  ...
  static async remove(id: number): Promise<AccessTokenModel> {
    const model = await db.deleteFrom('access_tokens').where('id', '=', id).executeTakeFirstOrThrow()
...
export async function all(limit = 10, offset = 0): Promise<AccessTokenType[]> {
  return await db
    .selectFrom('access_tokens')
    .selectAll()
...
export async function remove(id: number) {
  return await db.deleteFrom('access_tokens').where('id', '=', id).executeTakeFirst()
}
