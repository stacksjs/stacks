...
export type SubscriberEmailJsonResponse = Selectable<SubscriberEmailsTable>
...
export class SubscriberEmailModel {
  ...
  applyWhere<V>(column: keyof SubscriberEmailsTable, ...args: [V] | [Operator, V]): SubscriberEmailModel {
    if (args.length === 1) {
      const [value] = args
  ...
  where<V = string>(column: keyof SubscriberEmailsTable, ...args: [V] | [Operator, V]): SubscriberEmailModel {
