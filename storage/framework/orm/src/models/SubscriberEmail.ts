...
export class SubscriberEmailModel {
  ...
  static async remove(id: number): Promise<SubscriberEmailModel> {
    const model = await db.deleteFrom('subscriber_emails').where('id', '=', id).executeTakeFirstOrThrow()
  ...
  async orderBy(column: keyof SubscriberEmailType, order: 'asc' | 'desc'): Promise<SubscriberEmailType[]> {
    return await db.selectFrom('subscriber_emails').selectAll().orderBy(column, order).execute()
  }

  async orderByDesc(column: keyof SubscriberEmailType): Promise<SubscriberEmailType[]> {
    return await db.selectFrom('subscriber_emails').selectAll().orderBy(column, 'desc').execute()
  }

  async orderByAsc(column: keyof SubscriberEmailType): Promise<SubscriberEmailType[]> {
    return await db.selectFrom('subscriber_emails').selectAll().orderBy(column, 'asc').execute()
  }
  ...
  async update(subscriberemail: SubscriberEmailUpdate): Promise<Result<SubscriberEmailType, Error>> {
    if (this.subscriberemail.id === undefined) return err(handleError('SubscriberEmail ID is undefined'))
    ...
    if (!updatedModel) return err(handleError('SubscriberEmail not found'))
  ...
  async delete(): Promise<void> {
    if (this.subscriberemail.id === undefined) throw new Error('SubscriberEmail ID is undefined')

    await db.deleteFrom('subscriber_emails').where('id', '=', this.subscriberemail.id).execute()
  ...
  async refresh(): Promise<void> {
    ...
    if (!refreshedModel) throw new Error('SubscriberEmail not found')
...
export async function count(): Number {
  const results = await db.selectFrom('subscriber_emails').selectAll().execute()
...
export async function get(
  criteria: Partial<SubscriberEmailType>,
  sort: { column: keyof SubscriberEmailType; order: 'asc' | 'desc' } = { column: 'created_at', order: 'desc' },
) {
  let query = db.selectFrom('subscriber_emails')
...
export async function all(limit = 10, offset = 0): Promise<SubscriberEmailType[]> {
  return await db
    .selectFrom('subscriber_emails')
    .selectAll()
...
export async function recent(limit: number): Promise<SubscriberEmailModel[]> {
  return await db.selectFrom('subscriber_emails').selectAll().limit(limit).execute()
}

export async function last(limit: number): Promise<SubscriberEmailType> {
  return await db.selectFrom('subscriber_emails').selectAll().orderBy('id', 'desc').limit(limit).execute()
}

export async function update(id: number, subscriberemailUpdate: SubscriberEmailUpdate) {
  return await db.updateTable('subscriber_emails').set(subscriberemailUpdate).where('id', '=', id).execute()
}

export async function remove(id: number) {
  return await db.deleteFrom('subscriber_emails').where('id', '=', id).executeTakeFirst()
}
...
export async function whereIs(criteria: Partial<SubscriberEmailType>, options: QueryOptions = {}) {
  let query = db.selectFrom('subscriber_emails')
...
export async function whereIn(column: keyof SubscriberEmailType, values: any[], options: QueryOptions = {}) {
  let query = db.selectFrom('subscriber_emails')
