...
export class AuthorModel extends BaseOrm<AuthorModel, AuthorsTable, AuthorJsonResponse> {
  ...
  constructor(author: AuthorJsonResponse | undefined) {
    ...
    this.withRelations = []
    this.selectFromQuery = db.selectFrom('authors')
    this.updateFromQuery = db.updateTable('authors')
    this.deleteFromQuery = db.deleteFrom('authors')
...
set updated_at(value: string) {
  ...
  static async find(id: number): Promise<AuthorModel | undefined> {
    let query = db.selectFrom('authors').where('id', '=', id).selectAll()
  ...
  static async all(): Promise<AuthorModel[]> {
    ...
    const models = await db.selectFrom('authors').selectAll().execute()
  ...
  async applyCreate(newAuthor: NewAuthor): Promise<AuthorModel> {
    ...
    const result = await db.insertInto('authors')
      .values(filteredValues)
      .executeTakeFirst()

    const model = await db.selectFrom('authors')
 ...
 dispatch('author:deleted', model)
    ...
    const deleted = await db.deleteFrom('authors')
      .where('id', '=', this.id)
      .execute()
 ...
 dispatch('author:deleted', model)
    ...
    return await db.deleteFrom('authors')
      .where('id', '=', id)
      .execute()
  }
...
export async function remove(id: number): Promise<void> {
  await db.deleteFrom('authors')
    .where('id', '=', id)
    .execute()
}

export async function whereName(value: string): Promise<AuthorModel[]> {
          const query = db.selectFrom('authors').where('name', '=', value)
...
export async function whereEmail(value: string): Promise<AuthorModel[]> {
          const query = db.selectFrom('authors').where('email', '=', value)
          const results: AuthorJsonResponse = await query.execute()
