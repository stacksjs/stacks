...
export class UserModel {
  ...
  static async create(newUser: NewUser): Promise<UserModel> {
    const model = await db.insertInto('users').values(newUser).returningAll().executeTakeFirstOrThrow()
  ...
  static async remove(id: number): Promise<UserModel> {
    const model = await db.deleteFrom('users').where('id', '=', id).returningAll().executeTakeFirstOrThrow()
...
export async function create(newUser: NewUser) {
  return await db.insertInto('users').values(newUser).returningAll().executeTakeFirstOrThrow()
}
