...
export class DriverModel extends BaseOrm<DriverModel, DriversTable, DriverJsonResponse> {
  ...
  constructor(driver: DriverJsonResponse | undefined) {
    ...
    this.withRelations = []
    this.selectFromQuery = db.selectFrom('drivers')
    this.updateFromQuery = db.updateTable('drivers')
    this.deleteFromQuery = db.deleteFrom('drivers')
...
set updated_at(value: string) {
  ...
  static async find(id: number): Promise<DriverModel | undefined> {
    let query = db.selectFrom('drivers').where('id', '=', id).selectAll()
  ...
  static async all(): Promise<DriverModel[]> {
    ...
    const models = await db.selectFrom('drivers').selectAll().execute()
  ...
  async applyCreate(newDriver: NewDriver): Promise<DriverModel> {
    ...
    const result = await db.insertInto('drivers')
      .values(filteredValues)
      .executeTakeFirst()

    const model = await db.selectFrom('drivers')
 ...
 dispatch('driver:deleted', model)
    ...
    const deleted = await db.deleteFrom('drivers')
      .where('id', '=', this.id)
      .execute()
 ...
 dispatch('driver:deleted', model)
    ...
    return await db.deleteFrom('drivers')
      .where('id', '=', id)
      .execute()
  }
...
export async function remove(id: number): Promise<void> {
  await db.deleteFrom('drivers')
    .where('id', '=', id)
    .execute()
}

export async function whereName(value: string): Promise<DriverModel[]> {
          const query = db.selectFrom('drivers').where('name', '=', value)
...
export async function wherePhone(value: string): Promise<DriverModel[]> {
          const query = db.selectFrom('drivers').where('phone', '=', value)
          const results: DriverJsonResponse = await query.execute()
...
export async function whereVehicleNumber(value: string): Promise<DriverModel[]> {
          const query = db.selectFrom('drivers').where('vehicle_number', '=', value)
          const results: DriverJsonResponse = await query.execute()
...
export async function whereLicense(value: string): Promise<DriverModel[]> {
          const query = db.selectFrom('drivers').where('license', '=', value)
          const results: DriverJsonResponse = await query.execute()
...
export async function whereStatus(value: string | string[]): Promise<DriverModel[]> {
          const query = db.selectFrom('drivers').where('status', '=', value)
          const results: DriverJsonResponse = await query.execute()
