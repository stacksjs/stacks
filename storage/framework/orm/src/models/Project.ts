...
export class ProjectModel {
  ...
  static async create(newProject: NewProject): Promise<ProjectModel> {
    const model = await db.insertInto('projects').values(newProject).returningAll().executeTakeFirstOrThrow()
  ...
  static async remove(id: number): Promise<ProjectModel> {
    const model = await db.deleteFrom('projects').where('id', '=', id).returningAll().executeTakeFirstOrThrow()
...
export async function remove(id: number) {
  return await db.deleteFrom('projects').where('id', '=', id).returningAll().executeTakeFirst()
}
