 ...
 order_id?: string
     ...
     uuid?: string 
      ...
      export class TransactionModel {
        ...
        private readonly fillable: Array<keyof TransactionJsonResponse> = ["order_id","amount","status","payment_method","payment_details","transaction_reference","loyalty_points_earned","loyalty_points_redeemed","uuid"]
...
get payment_details(): string // Store as JSON string | undefined {
...
set payment_details(value: string // Store as JSON string) {
...
set updated_at(value: Date) {
      this.attributes.updated_at = value
        ...
        async applyPaginate(options: QueryOptions = { limit: 10, offset: 0, page: 1 }): Promise<TransactionResponse> {
          ...
          const transactionsWithExtra = await DB.instance.selectFrom('transactions')
            ...
            let nextCursor = null
            if (transactionsWithExtra.length > (options.limit ?? 10)) nextCursor = transactionsWithExtra.pop()?.id ?? null
 ...
 dispatch('transaction:deleted', model)
        ...
        static whereOrderId(value: string): TransactionModel {
          const instance = new TransactionModel(null)
...
...this.customColumns,
      ...
      export async function whereOrderId(value: string): Promise<TransactionModel[]> {
          const query = DB.instance.selectFrom('transactions').where('order_id', '=', value)
...
export async function wherePaymentDetails(value: string // Store as JSON string): Promise<TransactionModel[]> {
