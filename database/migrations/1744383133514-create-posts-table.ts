...
export async function up(db: Database<any>) {
  await db.schema
    ...
    .addColumn('post_category_id', 'integer', col =>
      col.references('post_categories.id').onDelete('cascade'))
  ...
  await db.schema.createIndex('posts_post_category_id_index').on('posts').column('post_category_id').execute()
