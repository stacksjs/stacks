import type { AuctionConfig } from '@stacksjs/types'

/**
 * **Auction Configuration**
 *
 * Controls the auction feature bundle (Auction / AuctionItem / Bid / Pledge
 * models, the proxy-bidding engine, anti-sniping and the settlement report).
 * Flip `enabled` to `false` to leave the bundle inert at boot. Manage via
 * `./buddy auction:install` / `./buddy auction:uninstall` rather than editing
 * this file by hand.
 *
 * Every amount is integer minor units (cents). The timing fields here are
 * defaults: an individual auction row overrides them, because a two-week online
 * catalogue and a ninety-minute in-room gala want different answers and neither
 * should have to be the global one.
 */
export default {
  enabled: true,
  ...
  /** Hard cap per lot, so a bidding war cannot keep the gala running to dawn. */
  maxExtensions: 20,
} satisfies AuctionConfig
